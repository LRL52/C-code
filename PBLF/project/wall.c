...
int main()
{
	...
	int sas,sa,score[3]={0},xuan=0,cishu=2;
	float ze,guanka;
	char str[100]={'\0'};
	done:
	printf( "1.选择关卡\n2.查看历史分数\n3.退出游戏\n" ); 
	sa=scanf( "\n%f", &ze );
	    ...
	    if (sa!=1)
	    {gets(str);printf("非法输入，请重输。\n");goto done;
		}
		if (ze==1){
		printf("总共三关，你已打通%d关，所以你可以选择玩前%d关。\n请输入你的选择（1、2、3）\n",xuan,xuan==3?3:xuan+1);
	           sas=scanf("\n%f",&guanka);
			    while(guanka>(xuan==3?3:xuan+1)||guanka<=0||guanka!=(int)guanka||sas!=1){gets(str);printf("输入非法，请重新输入。\n");scanf("\n%f",&guanka);}
	           N=guanka*2+1;}
...
| |_| | (_) | |_| |  \\ V  V /| | | | |_|\n\
  ...
  __/ |                                 \n\
 |___/   \n" );                               
         if(score[((int)guanka)-1]<(n+cishu+1000/difftime(end,start))) 
