...
void Switchtask(){ //切换关卡 
    while(1){
    	...
    	if(taskid < 1 || taskid > Data[id].taskid){
...
int main(){
    ...
    while(1){
		...
		if(op == 2){
			...
			printf("当前用户: %s\n", Data[id].username); 
			printf("\t\t\t\t     Username  名次  总分  猜数游戏  XX游戏  LRL52的博弈  XX游戏  长城游戏 \n"); 
			for(int i = 1; i <= n; ++i)
				printf("\t\t\t\t   %10s  %4d  %4d  %8d  %6d  %11d  %6d  %8d\n", Data[i].username, i, calc(i, 0), Data[i].data[1][1], Data[i].data[2][1], Data[i].data[3][1], Data[i].data[4][1], Data[i].data[5][1]);
			system("pause");  
		} 
