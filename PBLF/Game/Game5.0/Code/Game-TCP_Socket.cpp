...
static void ReadData(){
	if(init_socket() != 0){
		printf("Error：连接服务器失败，请稍后再尝试！");
		system("pause");
		exit(0); 
	}
	recvdata(wr);
	io = fopen("./data.txt", "w"); 
	fwrite(wr, 1, sizeof(wr), io);
    fclose(io);
    io = fopen("./data.txt", "r"); 
    if(io == NULL){
...
static void MainMenu(){
    ...
    while(1){
		...
		else if(op == 1 && Login()) break;
	    else if(op == 2){Register(); continue;}
	    else if(op == 3){WriteData(); exit(0);}
	}
}
