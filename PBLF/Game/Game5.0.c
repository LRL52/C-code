...
static void quicksort(int l,int r, int op)
{
    int mid = (l + r) >> 1;//中间数
    int i = l, j = r;
    do{
        while(calc(i, op) > calc(mid, op)) i++; //calc(int, int)支持不同关键字排名 
        while(calc(j, op) < calc(mid, op)) j--;
        if(i <= j){
        	if(id == i) id = j; //id记着一起变 
        	else if(id == j) id = i;
            node tmp = Data[i]; Data[i] = Data[j]; Data[j] = tmp;
            i++; j--;
        }
    }while(i <= j);
    if(l < j) quicksort(l, j, op);
    if(i < r) quicksort(i, r, op);
}
